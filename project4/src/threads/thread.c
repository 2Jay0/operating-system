...
bool comp_priority(const struct list_elem *first, const struct list_elem *second);
void calculate_all(int check);
int multi_float(int num1, int num2);
int divide_float(int num1, int num2);
               ...
               thread_func *function, void *aux) 
{
  ...
  if(priority>thread_get_priority()) // new thread priorty>runnning thread priority
	  thread_yield();
...
thread_set_nice (int nice UNUSED) 
{
  ...
  if(cur!=idle_thread)
  {
	  cur->priority=PRI_MAX - ((cur->recent_cpu/4)>>F_P) - (cur->nice *2);
...
{
  int64_t temp = ((int64_t)load_avg*100) / (1<<F_P);
  return temp;
}
...
{
  int64_t temp = ((int64_t)thread_current()->recent_cpu * 100) / (1<<F_P);
  return temp;
}

...
void calculate_all(int check)
{
	...
	if(check==1)
	{
		...
		load_avg = (((59*load_avg)+(ready_thread_count*(1<<F_P)))/60);
		...
		while(e != list_end(&all_list))
		{
			...
			if (t != idle_thread) 
			{
				t->recent_cpu=multi_float(divide_float(2*load_avg,(2*load_avg+(1<<F_P))),t->recent_cpu)+(t->nice)*(1<<F_P);
			}
			e = list_next(e);
		}
	}
	else if(check!=1)
	{
		...
		while(e!=list_end(&all_list))
		{
			...
			if(temp!=idle_thread)
			{
				temp->priority = PRI_MAX - ((temp->recent_cpu/4)>>F_P) - (temp->nice *2);
